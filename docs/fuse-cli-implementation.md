...
- [x] **1.6** Test: no arguments → usage message on stderr, exit 2.
...
- [x] **3.1** Create `stage1/fusec/src/color.rs`. Implement ANSI SGR
...
- [x] **3.2** Parse `--color auto|always|never` in `parse_args`.
      Default is `auto`. Invalid value → usage error, exit 2.

- [x] **3.3** Define error code registry. Errors: `E0001` type mismatch,
...
- [x] **3.6** Implement short-format renderer: single line per diagnostic,
      `file:line:col: severity[code]: message`. Designed for editor
      integration.

- [x] **3.7** Parse `--error-format short|long` in `parse_args`.
      Default is `long`. Invalid value → usage error, exit 2.

- [x] **3.8** Implement summary line after all diagnostics:
      `error: N errors found — compilation stopped` or
      `error: N errors, M warnings found — compilation stopped`.

- [x] **3.9** Test: long format matches spec layout. Test: short format
...
- [x] **4.4** Verify the evaluator (`evaluator.rs`) can be invoked from
...
- [x] **4.5** Test `--check` on a clean file → exit 0, empty stderr.
...
- [x] **5.1** Implement `--emit tokens <file>`: lex the file, print one
      token per line to **stdout**: `line:col  Kind  'text'`. Exit 0.
      If the file cannot be read, error on stderr, exit 1.

- [x] **5.2** Implement `--emit ast <file>`: parse the file, print the
...
- [x] **6.1** Implement `--repl`: print `fuse>` prompt, read a line,
...
- [x] **6.2** On check/eval error, print the diagnostic and continue
      the session — do not exit. The REPL is resilient to errors.

- [x] **6.3** `--repl` must not accept a file argument. `--repl` with
      a file → usage error, exit 2.

- [x] **6.4** Test: REPL evaluates `2 + 2` → prints `4`. Test: type
...
- [x] **7.1** Implement `--warn-unused`: the checker scans for bindings
...
- [x] **7.2** Implement `--deny-warnings`: if any warnings were produced,
      exit 1 instead of 0. Warnings are still printed — only the exit
      code changes.

- [x] **7.3** Test: `--warn-unused` on a file with unused bindings →
...
- [x] **8.1** Create `stage1/fusec/tests/cli_suite.rs`. Use the existing
      `harness.rs` infrastructure to invoke the `fusec` binary and
      capture stdout, stderr, and exit code.

- [x] **8.2** Meta tests: `--help` stdout + exit 0, `-h` alias,
      `--version` stdout + exit 0, `-V` alias.

- [x] **8.3** Usage error tests: no args → exit 2, unknown flag → exit 2,
...
- [x] **8.4** Check mode tests: clean file → exit 0 + empty stderr,
      error file → exit 1 + diagnostics on stderr, `--error-format short`
      produces parseable one-line output.

- [x] **8.5** Compile mode tests: clean file → exit 0 + binary exists,
      error file → exit 1 + no binary + diagnostics on stderr.

- [x] **8.6** Run mode tests: hello-world → stdout output + exit 0,
      file with errors → no output + diagnostics + exit 1.

- [x] **8.7** Emit mode tests: each stage produces non-empty stdout
      on a valid file.

- [x] **8.8** Flag combination tests: `--check --color never`,
      `--check --error-format short`, `--check --deny-warnings`.

- [x] **8.9** Run `cargo test` — all existing + new CLI tests green.
